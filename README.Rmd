...
We will use the following example data `demo` on 100 US city councilor names with 30\% of the names have verified ethnoracial information: 

```{r echo = F}
load('data/demo.rdata')
head(demo[,2:ncol(demo)])
```

## 1. Image-Based Prediction

We first want to collect images associated with each name and make predictions based on the images. We provide two python functions `img_search` and `img_pred` to do these steps. For users who don't have python installed in their computer, we also provide a Google Colab demonstration here:.  
